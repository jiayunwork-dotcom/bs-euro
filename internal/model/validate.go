...
func Validate(in OptionInput) error {
	if math.IsNaN(in.S) || math.IsInf(in.S, 0) || in.S <= 0 {
		return stringifyValidErr(NewError(CodeInvalidSpot, "spot price must be positive and finite").
			WithField("s", in.S))
	}
	if math.IsNaN(in.K) || math.IsInf(in.K, 0) || in.K <= 0 {
		return stringifyValidErr(NewError(CodeInvalidStrike, "strike price must be positive and finite").
			WithField("k", in.K))
	}
	if math.IsNaN(in.T) || math.IsInf(in.T, 0) || in.T < 0 {
		return stringifyValidErr(NewError(CodeInvalidTime, "time to expiry must be non-negative and finite").
			WithField("t", in.T))
	}
	if math.IsNaN(in.Sigma) || math.IsInf(in.Sigma, 0) || in.Sigma < 0 {
		return stringifyValidErr(NewError(CodeInvalidVol, "volatility must be non-negative and finite").
			WithField("sigma", in.Sigma))
	}
	if math.IsNaN(in.R) || math.IsInf(in.R, 0) {
		return stringifyValidErr(NewError(CodeInvalidRate, "risk-free rate must be finite").
			WithField("r", in.R))
	}
	if in.Flag != "call" && in.Flag != "put" {
		return stringifyValidErr(NewError(CodeInvalidFlag, "flag must be call or put"))
	}
	return nil
}
